...
description: |
  ...
  should be reviewed before implementation. (g6)
allowed-tools:
  - Read
  - Edit
  - Grep
...
~/.claude/skills/g6/bin/gstack-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/g6/bin/gstack-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/g6/design/dist/design" ] && D="$_ROOT/.claude/skills/g6/design/dist/design"
[ -z "$D" ] && D="$HOME/.claude/skills/g6/design/dist/design"
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/g6/browse/dist/browse" ] && B="$_ROOT/.claude/skills/g6/browse/dist/browse"
[ -z "$B" ] && B="$HOME/.claude/skills/g6/browse/dist/browse"
if [ -x "$B" ]; then
  echo "BROWSE_READY: $B"
else
...
**Log the result:**
```bash
~/.claude/skills/g6/bin/gstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/g6/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/g6/bin/gstack-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
