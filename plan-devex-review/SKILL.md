...
description: |
  ...
  (APIs, CLIs, SDKs, libraries, platforms, docs). (g6)
...
~/.claude/skills/g6/bin/gstack-timeline-log '{"skill":"plan-devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/g6/bin/gstack-question-log '{"skill":"plan-devex-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
\`~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md\`
...
SLUG=$(~/.claude/skills/g6/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
Read the `/office-hours` skill file at `~/.claude/skills/g6/office-hours/SKILL.md` using the Read tool.
...
Load the "## Pass 1" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`
...
**Anti-shortcut clause:** The plan file is the OUTPUT of the interactive review, not a substitute for it. Do not write every finding into one plan write and call ExitPlanMode without firing AskUserQuestion — the model must walk the user through each finding interactively. If you have ANY non-trivial finding in any review section, the path from finding to ExitPlanMode goes THROUGH AskUserQuestion. Zero findings in every section is the only path to ExitPlanMode that bypasses AskUserQuestion. If you find yourself wanting to write a plan with findings before asking, stop and call AskUserQuestion now.
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/g6/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/g6/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
```bash
eval "$(~/.claude/skills/g6/bin/gstack-slug 2>/dev/null)"
~/.claude/skills/g6/bin/gstack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.claude/skills/g6/plan-devex-review/dx-hall-of-fame.md`.
...
**Persist the result:**
```bash
~/.claude/skills/g6/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/g6/bin/gstack-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
