...
[ -z "$_AUTO" ] && _AUTO=$(~/.claude/skills/g6/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.claude/skills/g6/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.claude/skills/g6/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/g6/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.claude/skills/g6/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.claude/skills/g6"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".claude/skills/g6/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".claude/skills/g6"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".claude/skills/g6" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".claude/skills/g6"
elif [ -d "$HOME/.claude/skills/g6" ]; then
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/Bij4n/g6.git "$TMP_DIR/gstack"
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.claude/skills/g6" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.claude/skills/g6" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.claude/skills/g6"
  fi
fi
_TEAM_MODE=$(~/.claude/skills/g6/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .claude/skills/g6/ 2>/dev/null || true
if ! grep -qF '.claude/skills/g6/' .gitignore 2>/dev/null; then
  echo '.claude/skills/g6/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.claude/skills/g6/` when you're ready."
...
```bash
~/.claude/skills/g6/bin/gstack-update-check --force 2>/dev/null || \
.claude/skills/g6/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.claude/skills/g6/` when you're ready."
