...
version: 0.2.0
description: |
  Launch g6 Browser — AI-controlled Chromium with the sidebar extension baked in.
  Opens a visible browser window where you can watch every action in real time.
  The sidebar shows a live activity feed and chat. Anti-bot stealth built in.
  Use when asked to "open g6 browser", "launch browser", "connect chrome",
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/g6/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/g6/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/g6/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/g6/extension"
...
3. If the server IS healthy but the browser isn't visible, try `$B focus` (macOS only)
...
>
> **Window management:**
> - `$B focus` — bring Chrome to the foreground (macOS only; no-op on Linux/Windows)
