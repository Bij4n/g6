...
| [`/open-g6-browser`](#open-g6-browser) | **GStack Browser** | Launch GStack Browser with sidebar, anti-bot stealth, auto model routing, cookie import, and Claude Code integration. Watch every action live. |
...
| [`/g6-upgrade`](#g6-upgrade) | **Self-Updater** | Upgrade gstack to the latest version. Detects global vs vendored install, syncs both, shows what changed. |
...
`/browse` runs headless by default. You don't see what the agent sees. `/open-g6-browser` changes that. It launches GStack Browser (rebranded Chromium with anti-bot stealth) controlled by Playwright, with the sidebar extension auto-loaded. You watch every action in real time.
...
Keep gstack current with one command. It detects your install type (global at `~/.claude/skills/g6` vs vendored in your project at `.claude/skills/g6`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.
