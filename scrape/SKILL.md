...
description: |
  ...
  mutating flows (form fills, clicks, submissions), use $B primitives directly.
...
~/.claude/skills/g6/bin/gstack-timeline-log '{"skill":"scrape","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/g6/bin/gstack-question-log '{"skill":"scrape","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
clicking buttons that mutate state), refuse and route to direct `$B` primitives.
...
> "/scrape is read-only. For mutating flows, use $B click / $B fill / $B type directly."
...
- Mutating actions (use $B primitives directly)
...
```bash
~/.claude/skills/g6/bin/gstack-learnings-log '{"skill":"scrape","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
