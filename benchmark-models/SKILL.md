...
description: |
  ...
  "which model is best for X", "cross-model comparison", "model shootout". (g6)
...
~/.claude/skills/g6/bin/gstack-timeline-log '{"skill":"benchmark-models","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
BIN="$HOME/.claude/skills/g6/bin/gstack-model-benchmark"
[ -x "$BIN" ] || BIN=".claude/skills/g6/bin/gstack-model-benchmark"
...
```bash
[ -n "$ANTHROPIC_API_KEY" ] || grep -q 'ANTHROPIC' "$HOME/.claude.json" 2>/dev/null && echo "JUDGE_AVAILABLE" || echo "JUDGE_UNAVAILABLE"
```
